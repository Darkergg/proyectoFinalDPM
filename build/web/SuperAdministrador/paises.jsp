                            ...
                            <div class="card styled-dialog" >
                                <div class="card-header" style="background-color: #1cb2e2">
                                    <div class="d-flex justify-content-between " >
                                        <div class="text-center w-100"><h1 class="text-center">Tabla de Paises</h1></div>
                                ...
                                <div class="card-body">
                                    <div class="px-5" >
                                        ...
                                        <form action="../paisesController" method="POST" style="display: none;"  name="frmActuPais" id="frmActuPa">
                                            ...
                                            <input type="hidden" value="0" name="txtIdPais"/>
                                            <div class="mb-3 w-25">

                                                <label for="nombre" class="col-form-label">Actualizar País </label>
                                                <input type="text" class="form-control" id="txtActPais" name="txtActPais"  placeholder="País" required="Campo requerido">
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <a data-toggle="tooltip" title="Cancelar" data-placement="bottom" id='btnCancelar1' name¨="btnCancelar1"  onclick="$('#frmPais').hide();$('#btnNuevo').show(); $('#frmActuPa').hide();"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-x-circle-fill"></i> Cancelar</span></a>

                                            </div>

                                        </form>
                                        <%--  fromulario de insertar --%> 
                                        <form action="../paisesController" method="POST" style="display: none;"  name="frmPais" id="frmPais">
                                            <input type="hidden" value="0" name="txtIdPais"/>
                                            <div class="mb-3 w-25">
                                                <label for="nombre" class="col-form-label">Nuevo País:  </label>
                                                <input type="text" class="form-control" id="txtPais" name="txtPais"  placeholder="País" required="Campo requerido" >
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <a data-toggle="tooltip" title="Cancelar" data-placement="bottom" id='btnCancelar' name¨="btnCancelar"  onclick="$('#frmPais').hide();$('#btnNuevo').show(); $('#frmActuPais').hide();"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-x-circle-fill"></i> Cancelar</span></a>

                                            </div>
                                        </form>

                                    </div> 
                                    <a data-toggle="tooltip" title="Nuevo" data-placement="bottom" id='btnNuevo' name¨="btnNuevo"  onclick="$('#frmPais').show();
                                            $('#btnNuevo').hide();$('#txtPais').val('');
                                            $('#btnEdit').hide();$('#txtActPais').val('');
                                            $('#frmActuPa').hide();
                                            $('#btnEditar').hide();
                                    ...
                                    <table class="styled-table table-responsive-xl w-100" id="tabla">
                                        ...
                                        <tbody>
                                            <%
                                                List<Paises> lists = daoPa.getPaises();
                                                for (Paises pais : lists) {
                                            %>
                                            <tr class="text-center">
                                                ...
                                                <td p-0 pt-2 px-0">
                                                    <div class="row m-0 justify-content-center justify-content-md-start px-0">
                                                        <div class="col-sm-4 "style="margin-left: 16%;">
                                                            <a href="javascript:cargar(<%=pais.getIdPais()%>,'<%=pais.getPais()%>')" data-toggle="tooltip" title="Eliminar" data-placement="bottom" id='btnBorrar'><span class="material-symbols-outlined btn btn-danger" onclick="eliminar();">Delete</span></a>
                                                        </div>    
                                                        <div class="col-sm-4 p-0">
                                                            <a href="javascript:cargar(<%=pais.getIdPais()%>,'<%=pais.getPais()%>')" data-toggle="tooltip" title="Editar" data-placement="bottom" onclick="
                                                                    $('#btnNuevo').hide();
                                                                    $('#frmActuPa').show();
...
<script>
    function cargar(idPais, Pais)
    {
        document.frmActuPais.txtIdPais.value = idPais;
        document.frmActuPais.txtActPais.value = Pais;
    }
    ;
    function eliminar() {
        swal({
            title: 'Estas seguro?',
            text: 'Eliminará el País',
            icon: 'warning',
            buttons: true,
            dangerMode: true
        }).then((willDelete) => {
    ...
    ;
    function insertar() {
        var text = document.getElementById('txtPais').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',
                text: 'Insertar el País',
                icon: 'warning',
                buttons: true,
                dangerMode: true
            }).then((willDelete) => {
    ...
    function actualizar() {
        var text = document.getElementById('txtActPais').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',
                text: 'Actualizara el País',
                icon: 'warning',
                buttons: true,
                dangerMode: true
            }).then((willDelete) => {

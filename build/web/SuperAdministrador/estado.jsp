                            ...
                            <div class="card styled-dialog" >
                                <div class="card-header" style="background-color: #1cb2e2">
                                    <div class="d-flex justify-content-between " >
                                        <div class="text-center w-100"><h1 class="text-center">Tabla de Estados</h1></div>
                                ...
                                <div class="card-body">
                                    <div class="px-5" >
                                        ...
                                        <form action="../estadoController" method="POST" style="display: none;"  name="frmActuEst" id="frmActuEs">
                                            ...
                                            <input type="hidden" value="0" name="txtIdEstado"/>
                                            <div class="mb-3 w-25">

                                                <label for="nombre" class="col-form-label">Actualizar Estado: </label>
                                                <input type="text" class="form-control" id="txtActEstado" name="txtActEstado"  placeholder="Estado" required="Campo requerido">
                                            </div>
                                            <div class="mb-3">
                                        ...
                                        <form action="../estadoController" method="POST" style="display: none;"  name="frmEst" id="frmEs">
                                            <input type="hidden" value="0" name="txtIdEstado"/>
                                            <div class="mb-3 w-25">
                                                <label for="nombre" class="col-form-label">Nuevo Estado: </label>
                                                <input type="text" class="form-control" id="txtEstado" name="txtEstado"  placeholder="Estado" required="Campo requerido" >
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <a data-toggle="tooltip" title="Cancelar" data-placement="bottom" id='btnCancelar' name¨="btnCancelar"  onclick="$('#frmEs').hide();$('#btnNuevo').show(); $('#frmActuEs').hide();"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-x-circle-fill"></i> Cancelar</span></a>

                                            </div>
                                        </form>

                                    </div> 
                                    <a data-toggle="tooltip" title="Nuevo" data-placement="bottom" id='btnNuevo' name¨="btnNuevo"  onclick="$('#frmEs').show();
                                            $('#btnNuevo').hide();
                                            $('#btnEdit').hide();
                                            $('#frmActuEs').hide();
                                            $('#btnEditar').hide();
                                    ...
                                    <table class="styled-table table-responsive-xl w-100" id="tabla">
                                        ...
                                        <tbody>
                                            <%
                                                List<Estado> lista = daoEst.getEstado();
                                                for (Estado est : lista) {
                                            %>
                                            <tr class="text-center">
                                                <td> <%=est.getIdEstado()%></td>
                                                <td> <%=est.getEstado()%></td>
                                                <td p-0 pt-2 px-0">
                                                    <div class="row m-0 justify-content-center justify-content-md-start px-0">
                                                        <div class="col-sm-4 "style="margin-left: 16%;">
                                                            <a href="javascript:cargar(<%=est.getIdEstado()%>,'<%=est.getEstado()%>')" data-toggle="tooltip" title="Eliminar" data-placement="bottom" id='btnBorrar'><span class="material-symbols-outlined btn btn-danger" onclick="eliminar();">Delete</span></a>
                                                        </div>    
                                                        <div class="col-sm-4 p-0">
                                                            <a href="javascript:cargar(<%=est.getIdEstado()%>,'<%=est.getEstado()%>')" data-toggle="tooltip" title="Editar" data-placement="bottom" onclick="$('#frmTag').show();
                                                                    $('#btnNuevo').hide();
                                                                    $('#frmActuEs').show();
...
<script>
    function cargar(idEstado, estado)
    {
        document.frmActuEst.txtIdEstado.value = idEstado;
        document.frmActuEst.txtActEstado.value = estado;
    }
    ;
    function eliminar() {
        swal({
            title: 'Estas seguro?',
            text: 'Eliminara el estado',
            icon: 'warning',
            buttons: true,
            dangerMode: true
        }).then((willDelete) => {
    ...
    ;
    function insertar() {
        var text = document.getElementById('txtEstado').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',
                text: 'Insertar el estado',
                icon: 'warning',
                buttons: true,
                dangerMode: true
            }).then((willDelete) => {
    ...
    function actualizar() {
        var text = document.getElementById('txtActEstado').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',

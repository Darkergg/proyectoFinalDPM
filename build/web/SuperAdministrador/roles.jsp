                            ...
                            <div class="card styled-dialog" >
                                <div class="card-header" style="background-color: #1cb2e2">
                                    <div class="d-flex justify-content-between " >
                                        <div class="text-center w-100"><h1 class="text-center">Tabla de Roles</h1></div>
                                ...
                                <div class="card-body">
                                    <div class="px-5" >
                                        ...
                                        <form action="../rolController" method="POST" style="display: none;"  name="frmActuRol" id="frmActuRo">
                                            ...
                                            <input type="hidden" value="0" name="txtIdRol"/>
                                            <div class="mb-3 w-25">

                                                <label for="nombre" class="col-form-label">Actualizar Rol </label>
                                                <input type="text" class="form-control" id="txtActuaRol" name="txtActuaRol"  placeholder="Rol" required="Campo requerido">
                                            </div>
                                            <div class="mb-3">
                                                <a data-toggle="tooltip" title="Modificar" data-placement="bottom" id='btnModifi' name¨="btnModifi"  onclick="insertar()"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-arrow-clockwise"></i> Modificar</span></a>
                                                <a data-toggle="tooltip" title="Cancelar" data-placement="bottom" id='btnCancelar1' name¨="btnCancelar1"  onclick="$('#frmRol').hide();$('#btnNuevo').show(); $('#frmActuRo').hide();"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-x-circle-fill"></i> Cancelar</span></a>

                                            </div>

                                        </form>
                                        <%--  fromulario de insertar --%> 
                                        <form action="../rolController" method="POST" style="display: none;"  name="frmRol" id="frmRol">
                                            <input type="hidden" value="0" name="txtIdRol"/>
                                            <div class="mb-3 w-25">
                                                <label for="nombre" class="col-form-label">Nuevo Rol:   </label>
                                                <input type="text" class="form-control" id="txtRol" name="txtRol"  placeholder="Rol" required="Campo requerido" >
                                            </div>
                                            <div class="mb-3">
                                                ...
                                                <a data-toggle="tooltip" title="Cancelar" data-placement="bottom" id='btnCancelar' name¨="btnCancelar"  onclick="$('#frmRol').hide();$('#btnNuevo').show(); $('#frmActuRo').hide();"><span class="material-symbols-outlined btn " style="background-color: #1cb2e2"><i class="bi bi-x-circle-fill"></i> Cancelar</span></a>

                                            </div>
                                        </form>

                                    </div> 
                                    <a data-toggle="tooltip" title="Nuevo" data-placement="bottom" id='btnNuevo' name¨="btnNuevo"  onclick="$('#frmRol').show();
                                            $('#btnNuevo').hide();
                                            $('#txtRol').val('');
                                            $('#btnEdit').hide();
                                            $('#txtActuRol').val('');
                                                ...
                                                <td p-0 pt-2 px-0">
                                                    <div class="row m-0 justify-content-center justify-content-md-start px-0">
                                                        <div class="col-sm-4 "style="margin-left: 16%;">
                                                            <a href="javascript:cargar(<%=rol.getIdRol()%>,'<%=rol.getRol()%>')" data-toggle="tooltip" title="Eliminar" data-placement="bottom" id='btnBorrar'><span class="material-symbols-outlined btn btn-danger" onclick="eliminar();">Delete</span></a>
                                                        </div>    
                                                        <div class="col-sm-4 p-0">
                                                            <a href="javascript:cargar(<%=rol.getIdRol()%>,'<%=rol.getRol()%>')" data-toggle="tooltip" title="Editar" data-placement="bottom" onclick="
                                                                    $('#btnNuevo').hide();
                                                                    $('#frmActuRo').show();
...
<script>
    function cargar(idRol, rol)
    {
        document.frmActuRol.txtIdRol.value = idRol;
        document.frmActuRol.txtActuaRol.value = rol;
    }
    ;
    function eliminar() {
        swal({
            title: 'Estas seguro?',
            text: 'Eliminará el Rol',
            icon: 'warning',
            buttons: true,
            dangerMode: true
        }).then((willDelete) => {
    ...
    function insertar() {
        var text = document.getElementById('txtRol').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',
                text: 'Insertar el Rol',
                icon: 'warning',
                buttons: true,
                dangerMode: true
            }).then((willDelete) => {
    ...
    function actualizar() {
        var text = document.getElementById('txtActuaRol').value;
        if (text !== "") {
            swal({
                title: 'Estas seguro?',
                text: 'Actualizara el rol',
                icon: 'warning',
                buttons: true,
                dangerMode: true
            }).then((willDelete) => {
